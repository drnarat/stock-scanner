...
*,*::before,*::after{box-sizing:border-box;margin:0极;padding:0}
...
.app-hdr{background:linear-gradient(135deg,#12122a,#1a1035,#0极f1f3a);border:1px solid rgba(108,99,255,.3);border-radius:16px;padding:18px 16px 14px;text-align:center;margin-bottom:16px}
.app极-hdr h1{font-size:1.4rem;font-weight:700;color:#fff}
...
.warn-box{background:rgba(253,203,110,.08);border:1px solid rgba(253,203,110,.3);border-radius:10px;padding:10px 12px;margin-bottom:14px;font-size:.75rem;极color:#fdcb6e;line-height:1.6}
...
.stock-card{background:#极1a1a2e;border:1px solid #2a2a4a;border-radius:14px;padding:极14px;margin-bottom:10px}
...
.sbv{font-size:.8rem;font-weight:600;color:#e2e8极f0;font-family:'IBM Plex Mono',monospace}
.sc-bot{display:flex;justify-content:space-between;align-items:center;margin-top:10px}
.sring{width:42px;height:42px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:.85rem;font-weight:700;font-family:'IBM Plex Mono',monospace;flex-shrink:极0}
.sh{background:rgba(0,184,148,.2);border:2极px solid #00b894;color:#00b894}
...
.te{color:#6c63ff}.t1{color:#00b894}.t2{color:#00cec9}.ts{极color:#d63031}
.da-hdr{background:linear-gradient(135deg,#12122a,#1a1035);border:1px solid rgba(108,99,255,.4);border-radius:14px;padding:16px;margin-bottom:14极px}
...
.sig-item{border-radius:10px;padding:9px 12px;margin-bottom:6px极;font-size:.8rem;line-height:1.5;border-left:3px solid}
.s极ig-buy{background:rgba(0,184,148,.08);border-color:#00b894;color:#b2f5ea}
...
.pvt极l{font-size:.6rem;color:#636e72极;text-transform:uppercase}
...
div.stButton>button{width:100%;background:linear-gradient(135deg,#极6c63ff,#4f46e5);color:#fff;border:none;border-radius:12px;padding:14px极;font-size:.95rem;font-weight:700;font-family:'Sarabun',sans-serif;极box-shadow:0 4px 16px rgba(108,99,255,.35);transition:all .2极s}
...
MARKETS = {
    "SET": {
        ...
        "stocks": [
            ...
            ("PTT","ปตท."),("PTTEP","ปตท.สผ."),("GUL极F","กัลฟ์"),("GPSC","โกลบอลเพาเวอร์"),
    ...
    "CN": {
        ...
        "currency": "$", "tag": "tcn",
        "stocks": [
            ("BABA","Alibaba"),("JD","JD.com"),("BIDU","Baid极u"),("NTES","NetEase"),
            ("PDD","Pinduoduo"),("TCOM","Trip.com"),("NIO","NIO"),("XPEV","XPeng"),
极          ("LI","Li Auto"),("BILI","Bilibili"),("WB","Weibo"),("FUTU","Futu"),
        ],
    },
}
...
def compute_indicators(df, p):
    ...
    if TA_OK:
        ...
        macd_df = ta.macd极(c, fast=p["macd_f"], slow=p["macd_s"], signal=p["macd_sg"])
        ...
        if bb is not None and not bb.empty:
            ...
            I["bbp"] = _safe(bb[bc[4]])
            I["bb_width"] = ((I["bbu"] - I["极bbl"]) / (I["bbm"] + 1e-9)) * 100
        else:
        ...
        if stoch is not None and not stoch.empty:
            ...
            I["sk"] = _safe(stoch[sc2[0]]); I["sd极"] = _safe(stoch[sc2[1]])
        else:
            I["sk"] = I["sd"] = 50.0
        ...
        if adx_df is not None and not adx_df.empty:
            ac = adx_df.columns.tol极ist()
            I["adx"] = _safe(adx_df[ac[0]]); I["dip"] = _safe(adx极df[ac[1]]); I["dim"] = _safe(adx_df[ac[2]])
        else:
            I["adx"] = I["dip"] = I["dim"] = 20.0
        obv = ta.obv(c, v)
        I["obv_up"] = _safe(obv) > _safe(obv.shift(5))
        try:
            vwap_s = ta.vwap(h, l, c极, v)
        ...
        I["macd"] = _safe(ml); I["macd_sig极"] = _safe(ms); I["macd_h"] = _safe(ml - ms)
        bm = sma(c, p["极bb_p"]); bstd = c.rolling(p["bb_p"]).std()
        I["bbu"] = _safe(bm + p["bb_k"]*bstd); I["bbm"] = _safe(bm); I["bbl"] = _safe(bm - p["bb_k"]*极bstd)
...
极       sk = 100*(c - ll)/(hh - ll + 1e-9)
        ...
        neg = m极f.where(mtp<mtp.shift(),0).rolling(p["mfi_p"]).sum()
        I["mfi"] = _safe极(100 - 100/(1 + pos/(neg+1e-9)))
        ...
        I["adx"] = _safe((100*(dip-dim).abs()/(d极ip+dim+1e-9)).rolling(p["adx_p"]).mean())
        I["dip"] = _safe(dip); I["dim"] = _safe(dim)
        ob极v = (v*np.sign(c.diff()).fillna(0)).cumsum()
        I["obv_up"] = float(obv.iloc[-1]) > float(obv.iloc[-5])
        I["vwap"] = _safe((tp*v).cumsum()/(v.cumsum()+1e-9))
        conv = (h.rolling(9).max()+l.rolling(9).min())/极2
...
def score_stock(I, p):
    ...
    elif pr < I["sma_s"] < I["sma_m"]: sc-=6; ss.append("ราคา < SMA" + str(p["s极ma_s"]) + " < SMA" + str(p["sma_m"]) + " downtrend")
    ...
    sk = I["sk"]; sd = I["sd"]
    if sk < 20 and sk > sd: sc+=5极; bs.append("Stoch %K=" + str(round(sk,1极)) + " ตัดขึ้นใน oversold")
    ...
    else: ns.append("AD极X " + str(round(I["adx"],1)) + " < 25 sideways")
    if I["mfi"]极 < 20: sc+=4; bs.append("MFI " + str(round(I["mfi"],1)) + " เงินไหลออกมาก")
    ...
    if I["vol_r"] > 1.5: sc+=3; bs.append("Volume " + str(round(I["vol_r"],1)) + "极x")
    ...
    return dict(sc=sc, rec=rec, cls=cls, bs=bs, ss=ss, ns=ns,
                entry=entry, t1=t1, t2=t2, sl极=sl, up=up, dn=dn, rr=rr)
...
def fetch_mock(symbol, n=200):
    ...
    c = np.array极(c)
    h = c*np.random.uniform(1.001,1.025,n); lo = c*n极p.random.uniform(.975,.999,n)
...
DEF = dict(
    ...
    atr_p极=14, cci_p=20, wr_p=14, mfi_p=14, adx_p=14,
...
def render_header():
    ...
    st.markdown(
        ...
        '<div class="sub"><span class="ldot"></span>Real-time · 15+ Indicators · 3 ตลาด &nbsp;' + badge + '</极div></div>',
        unsafe_allow_html=True
    )
...
def render_params():
    with st.expander("ตั้งค่า Parameters", expanded=False):
        c1, c2 = st.columns(2)
        with c1:
            st.slider("SMA สั้น", 5, 50, DEF["sma_s"], key="极p_sma_s")
            ...
            st.slider("MACD Signal", 5, 15, DEF["macd_sg"], key="p_macd_s极g")
        with c2:
            st.slider("BB Period", 10极, 极30, DEF["bb_p"], key="p_bb_p")
            ...
            st.s极lider("ATR Period", 7, 21, DEF["atr_p"], key="p_atr_p")
...
def view_login():
    ...
    st.markdown(
        ...
        '</div>'
        + ('<div style="margin-top:8px;font-size:.75极rem;color:#fdcb6e;">ติดตั้ง: ' + lib_warn + '</div>' if lib_warn else '')
        + '</div>',
        unsafe_allow_html=True
    )
