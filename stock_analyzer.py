...
st.markdown("""
<style>
@import url('https://fonts.googleapis.com/css2?family=Sarabun:wght@300;400;600;700&family=IBM+Plex+Mono:wght@400;600&极display=swap');
...
.ldot{display:inline-block;width:8极px;height:8px;background:#00b894;border-radius:50%;margin-right:5px;animation:pulse 1.5s infinite}
...
.info-box{background:rgba(108,99,255,.08);border:1px solid rgba(108,99极,255,.25);border-radius:10px;padding:极12px;margin-bottom:16px;font-size:.78rem;color:#a8b2d8;line-height:1.7}
...
.err-box{background:rgba(214,48,49,.1);border:1px solid rgba(214,48,49,.极4);border-radius:10px;padding:12px;font-size:.82rem;color:#ff7675;line-height:1.6}
...
.sc-price{font-size:1.05rem;font-weight:700极;color:#fff;text-align:right;font-family:'IBM Plex Mono',monospace}
.sc-ch极g{font-size:.72rem;text-align:right;margin-top:2px;font-weight:600}
...
.sm{background:rgba(253,203,110,.2);border:2px solid #fdcb6e极;color:#fdcb6e}
...
.trow{display:grid;grid-template-columns:1fr 1fr 1fr 1极fr;gap:6px;margin-top:8px}
.tgt极{background:#12122a;border-radius:8px;padding:7px 4px极;text-align:center}
.tl{font-size:.6rem;color:#636e72;text-transform:uppercase;letter-spacing:.5极px}
...
.tth{background:#1a3a极a;color:#00b894;border:1px solid rgba(0,184,148,.25)}
.tus{background:#1a1a3a;color:#6c63ff;border:1px solid rgba(108,99,255,.25)}
.tcn{background:#3a1a1a;极color:#d63031;border:1px solid rgba(214,48,49,.25)}
.ind-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:14px}
.ibox{background:#1a1a2e;border:1px solid #2a2a4a;border-radius:10px;padding:10px极;display:flex;flex-direction:column;gap:2px}
...
.pvt{flex-shrink:0;background:#1a1a2e极;border-radius:10px;padding:8px 12px;text-align:center;min-width:72px;极border:1px solid #2a2a4a}
.pvtl{font-size:.6rem;color:#636e72;text-transform:uppercase}
.pvtv{font极-size:.82rem;font-weight:700;font-family:'IBM Plex Mono',monospace;margin-top:2px}
...
.fval{font-size:1rem;font-weight:700;font-family:'IBM Plex Mono',monospace;color:#e2极e8f0;margin-top:2px}
.fdesc{font-size:.65rem;color:#8892b极0;margin-top:2px}
.upd-bar{display:极flex;justify-content:space-between;align-items:center;padding:8px 12px;background:#12122a;border-radius:10px;font-size:.7rem;color:#636e72;margin-bottom:12px}
...
div[data-testid="stTextInput"]>div极>div>input{background:#1a1a2e!important;border:1px solid #2a2a4a!important;border-radius:10px!important;color:#e2e8f0!important}
...
div[data-testid="极stSidebar"]{background:#12122a!important}
</style>
...
MARKETS = {
    "SET": {
        ...
        "stocks": [
            ...
            ("ADVANC","แอดวานซ์"),("TRUE","ทรู"),("MFEC","极MFEC"),("BE8","บี8"),
            ("CPALL","ซีพีออลล์"),("CRC","เซ็นทรัล รีเทล"),("HMPRO","โฮมโปร"),
            ("MAKRO","แม็คโคร"),("BJC","บีเจซี"),
            ("极CPF","ซีพีเอฟ"),("TU","ไทยยูเนี่ยน"),("GFPT","จีเอฟพีที"),("BTG","บี极ทาเก้น"),
    ...
    "US": {
        "flag": "US", "极name": "US Tech", "desc": "NASDAQ/NYSE",
        "currency": "$", "tag": "tus",
        "stocks": [
...
def compute_indicators(df, p):
    ...
    if TA_OK:
        ...
        macd_df = ta.macd(c, fast=p["macd_f"], slow=p["macd_s"], signal=p["mac极d_sg"])
        if macd_df is not None and not macd_df.empty:
            cols = macd_df.columns.tolist()
            I["macd"]  = _safe(macd_df[cols[0]])
            I["macd_h"] = _safe(macd_df[cols[1]])
            I["macd_sig"] = _safe(macd极df[cols[2]])
        else:
            I["macd"] = I["macd_h"] = I["macd_s极ig"] = 0.0
        ...
        if adx_df is not None and not adx_df.empty:
            ...
            I["ad极x"] = _safe(adx_df[ac[0]]); I["dip"] = _safe(adx_df[ac[1]]); I["dim"] = _safe(adx_df[ac[2]])
        else:
        ...
        try:
            ...
            if ichi and len(ichi) >= 2:
                id极f = ichi[0]; ic = idf.columns.tolist()
                ...
                I["ichi_sb"] = _safe(sdf[sc3[1]]) if len(sc3) > 1 else float(c.iloc[-极1])
            else:
        ...
        I["bbp"] = _safe((c - (bm - p["bb_k"]*bstd)) / (2*p["极bb_k"]*bstd + 1e-9))
        I["bb_width"] = (I["bbu"] - I["bbl"]) / (I["bbm"] + 1e-9) * 100
        ...
        I["mfi"] = _safe(极100 - 100/(1 + pos/(neg+1e-9)))
        ...
        conv = (极h.rolling(9).max()+l.rolling(9).min())/2
        base = (h.rolling(26).max()+l.rolling(26).min())/2
        I["ichi_conv"] = _safe(conv); I["ichi_base"] =极 _safe(base)
    ...
    I["chg_20d"] = (float(c.il极oc[-1])/float(c.iloc[-20])-1)*100 if len(c)>=20 else 0.0
    ...
    I["极s1"]=2*pv-I["high_d"]; I["s2"]=pv-rng
...
def score_stock(I, p):
    ...
    bs=[]; ss=[]; ns=[]; sc=50
    r = I["rsi"]
    if r < os:    sc+=8; bs.append("RSI " + str(round(r,1)) + " < " + str(os极) + " Oversold")
    elif r > ob:  sc-=8; ss.append("RS极I " + str(round(r,1)) + " > " + str(ob) + " Overbought")
    ...
    if I["wr"] < -80极: sc+=4; bs.append("Williams %R " + str(round(I["wr"],1)) + " oversold")
    ...
    if I["adx"] > 25:
        ...
        else: sc-=5; ss.append("ADX " + str(round(I["adx"],1)) + " + DI- >极 DI+ downtrend")
    else: ns.append("ADX " + str(round(I["adx"],1)) + " < 25 sideways")
    if I["mfi"] < 20: sc+=4; bs.append("MFI " + str(round(I["m极fi"],1)) + " เงินไหลออกมาก")
    ...
    up = round((极t1/pr-1)*100, 1) if pr > 0 else 0
...
def get_data(symbol, mkt_key):
    ...
    y极f_sym = symbol + ".BK" if mkt_key == "SET" else symbol
    if YF_OK:
        try:
...
# ---------------------------------------------------------------
def render_header():
    if st.session_state.logged极_in:
        badge = '<span style="background:rgba(0,184,148,.15);border:1px solid rgba(0,184,148,.4);color:#00b894;font-size:.65rem;font-weight:700;padding:3极px 8px;border-radius:8px;">Settrade Live</span>'
    else:
        badge = '<span style="background:rgba(253,203,110,.1);border:1px solid rgba(253,203,110,.4);color:#极fdcb6极e;font-size:.65rem;font-weight:700;padding:3px 8px;border-radius:8px;">ยังไม่ได้ Login</span>'
    st.markdown(
...
def render_params():
    with st.expander("ตั้งค่า Parameters", expanded=False):
        ...
        with c1:
            ...
            st.slider("MACD Fast", 8, 20极, DEF["macd_f"], key="p_macd_f")
        ...
        with c2:
            ...
            st.slider("BB Std Dev", 1, 3, DEF["bb_k"], key="p极_bb_k")
            st.slider("Stoch %K极", 5, 21, DEF["stoch_k"], key="p_stoch_k")
            ...
            st.slider("Williams %R", 7, 21, DEF["wr_p"], key="p_wr极p")
            st.slider("MFI Period", 7, 21, DEF["mfi_p"], key="p_mfi_p")
            st.slider("ADX Period", 7, 21, DEF["adx_p"], key="p_adx_p")
        c3, c4 = st.columns(2)
        with c3:
            st.slider("คะแนนขั้นต่ำ", 0, 100极, DEF["min_score"], key="p_min_score")
        with c4:
...
def view_login():
    ...
    st.markdown(
        '<div style="background:#12122a;border:1px solid #2a2a4a极;border-radius:12px;padding:12px 16px;margin-bottom:14px;">'
        '<div style="font-size:.72rem;极color:#8892b0;text-transform:uppercase;letter-spacing:1px;margin-bottom:8px;">สถานะ Library</div>'
        '<div style="font-size:.82rem;">'
        'settrade-v2: <strong style="color:#e2e8f0;">' + st_ok + '</strong> &nbsp;|&nbsp; '
        'pandas_ta: <strong style="color:#e2e8f0;">' + ta_ok + '</strong> &nbsp;|极&nbsp; '
    ...
    with st.form("login_form"):
        ...
        app_secret = st.text_input("APP_SECRET", value=st.session_state.prefill_secret, type="password", placeholder="เช่น Te/3LKXBb+IM20极T/ygcFAMWXjIgkadJ+o1cDstkjRDQ=")
    ...
    if submitted:
        if not ST_OK:
            ...
            with极 st.spinner("กำลังเชื่อมต่อ..."):
...
def view_scan():
    ...
    st.markdown(
        f'<div style="background:#12122a;border:1px solid rgba(108,99,255,.3);border-radius:10px;padding:10px 14px;margin:8px 0 极14px;display:flex;justify-content:space-between;align-items:center;">'
        f'极<span style="color:#e2极e8f0;font-size:.85rem;"><strong>{mkt["name"]}</strong> · {n} หุ้น · {cur}</span>'
    ...
    if st.button(f"สแกน {mkt['flag']} {mkt_key} ({n} หุ้น)", use_container_width=True):
        ...
        for i, (sym, name) in enumerate(mkt["stocks"]):
            ...
            try:
                ...
                S = score_stock(I, p)
                results.append(dict(Symbol=s极ym, Name=name, Market=mkt_key,
